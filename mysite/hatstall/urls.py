...
urlpatterns = [
    url(r'^shdb', views.get_shdb_params, name='shdb'),
    url(r'^organizations/', include('hatstall.orgs_urls')),
    url(r'^about/', include('hatstall.about_urls')),
    ...
    url(r'^(?P<identity_id>[0-f]+)/update_enrollment/(?P<organization>[0-9A-Za-z\w|\W]+)$',
        views.update_enrollment, name='update identity'),
    url(r'^(?P<identity_id>[0-f]+)/enroll_in/(?P<organization>[0-9A-Za-z\w|\W]+)$',
        views.enroll_to_profile, name='enroll identity'),
    url(r'^(?P<identity_id>[0-f]+)/unenroll_from/(?P<organization_info>[0-9A-Za-z\w|\W]+)$',
